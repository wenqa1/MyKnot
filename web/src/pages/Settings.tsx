        ...
        <div className="space-y-5">
          <div className="flex flex-col items-center gap-2">
            <button onClick={() => fileInputRef.current?.click()} className="relative group" disabled={avatarUploading}>
              ...
              {avatarUploading && <div className="absolute inset-0 rounded-2xl bg-white/60 dark:bg-slate-800/60 flex items-center justify-center"><div className="w-5 h-5 border-2 border-knot-rose border-t-transparent rounded-full animate-spin" /></div>}
            </button>
          ...
          <label className="block"><span className="text-sm font-medium text-knot-dark">名字</span><input type="text" value={profileName} onChange={(e) => setProfileName(e.target.value)} placeholder="你的名字" className="mt-1.5 w-full px-4 py-3 bg-stone-50 dark:bg-slate-800 rounded-xl border border-stone-100 dark:border-slate-700 text-knot-dark dark:text-slate-200 placeholder:text-stone-300 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-knot-rose/40 transition-all" /></label>
          <button onClick={handleSaveProfile} disabled={profileSaving} className="w-full py-2.5 bg-stone-100 dark:bg-slate-700 text-knot-dark dark:text-slate-200 rounded-xl text-sm font-semibold hover:bg-stone-200 dark:hover:bg-slate-600 disabled:opacity-40 transition-all">{profileSaving ? "保存中..." : "保存名字"}</button>
          <hr className="border-stone-100 dark:border-slate-700" />
